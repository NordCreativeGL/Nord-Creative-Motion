...
export default function FjordHeroScene({ children }: { children?: React.ReactNode } = {}) {
  ...
  useEffect(() => {
    ...
    function buildIceberg(): THREE.Group {
      ...
      // keel cap — fills the open cross-section at waterline when camera is submerged
      {
    ...
    function init3d() {
      ...
      const tick = (t: number) => {
        ...
        try {
          ...
          if (scrollRevealed) {
            const scrollEl = document.getElementById('fj-scroll')
            if (scrollEl) scrollEl.style.opacity = String(Math.max(0, 1 - dive * 10))
          }
  ...
  const t = {
    en: {
      s1h: 'Websites & apps\nfor your business', s1hMobile: 'Websites and apps\nfor your business', s1b: 'Custom websites for businesses that want to stand out.',
    ...
    da: {
      s1h: 'Hjemmesider & apps\ntil din virksomhed', s1hMobile: 'Hjemmesider og apps\ntil din virksomhed', s1b: 'Skræddersyede hjemmesider til virksomheder der vil skille sig ud.',
      ...
      <div id="fj-copy" style={{ position: 'absolute', left: 0, right: 0, top: '14%', zIndex: 4, pointerEvents: 'none', opacity: 0, transition: 'opacity 0.6s ease' }}>
        <div id="fj-sec-0" style={{ position: 'absolute', left: 0, right: 0, top: 0, textAlign: 'center', padding: '0 6%', opacity: 1, willChange: 'opacity, transform' }}>
          <div style={{ width: 'fit-content', maxWidth: '86vw', margin: '0 auto', padding: 'clamp(18px, 2vw, 28px) clamp(22px, 2.4vw, 36px)' }}>
            <h1 style={{ fontFamily: "var(--font-geist-sans), sans-serif", fontWeight: 300, fontSize: isMobile ? 'clamp(28px, 7.5vw, 34px)' : 'clamp(36px,3.5vw,80px)', lineHeight: 1.05, letterSpacing: '.04em', textTransform: 'uppercase', color: '#f4fbff', margin: 0, whiteSpace: 'pre-line' }}>{isMobile ? t[lang].s1hMobile : t[lang].s1h}</h1>
        ...
        <div id="fj-sec-1" style={{ position: 'absolute', left: 0, right: 0, top: 0, textAlign: 'center', padding: '0 6%', opacity: 0, willChange: 'opacity, transform' }}>
          <div style={{ width: 'fit-content', maxWidth: '86vw', margin: '0 auto', padding: 'clamp(18px, 2vw, 28px) clamp(22px, 2.4vw, 36px)' }}>
            <h2 style={{ fontFamily: "var(--font-geist-sans), sans-serif", fontWeight: 300, fontSize: isMobile ? 'clamp(28px, 7.5vw, 34px)' : 'clamp(36px,3.5vw,80px)', lineHeight: 1.06, letterSpacing: '.04em', textTransform: 'uppercase', color: '#f4fbff', margin: 0 }}>{t[lang].s2h}</h2>
            <p style={{ fontFamily: "var(--font-geist-sans), sans-serif", fontWeight: 300, fontSize: 'clamp(1.125rem,1.15vw,1.5rem)', lineHeight: 1.6, color: 'rgba(230,236,240,0.85)', maxWidth: '900px', margin: '18px auto 0' }}>{t[lang].s2b}</p>
          </div>
        </div>
        <div id="fj-sec-2" style={{ position: 'absolute', left: 0, right: 0, top: 0, textAlign: 'center', padding: '0 6%', opacity: 0, willChange: 'opacity, transform' }}>
          <div style={{ width: 'fit-content', maxWidth: '86vw', margin: '0 auto', padding: 'clamp(18px, 2vw, 28px) clamp(22px, 2.4vw, 36px)' }}>
            <h2 style={{ fontFamily: "var(--font-geist-sans), sans-serif", fontWeight: 300, fontSize: isMobile ? 'clamp(28px, 7.5vw, 34px)' : 'clamp(36px,3.5vw,80px)', lineHeight: 1.06, letterSpacing: '.04em', textTransform: 'uppercase', color: '#f4fbff', margin: 0 }}>{t[lang].s3h}</h2>
            <p style={{ fontFamily: "var(--font-geist-sans), sans-serif", fontWeight: 300, fontSize: 'clamp(1.125rem,1.15vw,1.5rem)', lineHeight: 1.6, color: 'rgba(230,236,240,0.85)', maxWidth: '560px', margin: '18px auto 0' }}>{t[lang].s3b}</p>
          </div>
        </div>
      </div>

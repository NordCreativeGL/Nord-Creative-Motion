...
export default function WebOfferBg() {
  ...
  useEffect(() => {
    ...
    scene.background = new T.Color('#040e16')
    scene.fog = new T.FogExp2('#040e16', 0.008)

    const camera = new T.PerspectiveCamera(52, W() / H(), 0.1, 1200)

    scene.add(new T.AmbientLight('#0a2535', 1.2))
    const sun = new T.DirectionalLight('#b8d8e8', 1.4)
    sun.position.set(-20, -30, 40)
    scene.add(sun)
    const glowLight = new T.PointLight('#46c8ba', 3.5, 120)
    glowLight.position.set(0, 10, 0)
    scene.add(glowLight)
    const deepLight = new T.PointLight('#08384a', 2.0, 200)
    deepLight.position.set(0, -40, 0)
    scene.add(deepLight)

    const berg = buildIceberg()
    berg.position.set(0, 0, 0)
    scene.add(berg)

    const water = new T.Mesh(
      new T.PlaneGeometry(800, 800).rotateX(-Math.PI / 2),
      new T.MeshStandardMaterial({ color: '#08202c', transparent: true, opacity: 0.52, roughness: 0.1, metalness: 0.3 })
    )
    water.position.y = 0
    scene.add(water)
    ...
    const tick = () => {
      ...
      camera.position.set(Math.sin(angle) * 64, -10, Math.cos(angle) * 64)
      camera.lookAt(0, -4, 0)
  ...
  return <div ref={hostRef} style={{ position: 'absolute', inset: 0, zIndex: 0 }} />
}

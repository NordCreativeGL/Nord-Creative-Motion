...
function createDepth(canvas: HTMLCanvasElement): DepthPainter {
  ...
  function plan(w: number, h: number): Plan {
    ...
    if (!stack) {
      ...
      const k = Math.min((wl - 30) / maxAAll, (h - wl - 36) / maxBAll, w * 0.00165)
      ...
      DEPTH_BERGS.forEach((bd, i) => {
        ...
        const k = Math.min((sh * 0.30 - 24) / bMaxA(bd), (sh * 0.70 - 40) / bMaxB(bd), w * 0.0042)
  ...
  return (
    <section id={id} ref={sectionRef} style={{ position: 'relative', minHeight: '100dvh', overflow: 'hidden', background: '#000507' }}>
      ...
      <div style={{
        ...
        height: '200px',
        background: 'linear-gradient(to bottom, transparent, #041c29)',
        zIndex: 5,
        pointerEvents: 'none',
      }} />
    </section>

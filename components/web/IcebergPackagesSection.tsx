...
function createDepth(canvas: HTMLCanvasElement, allowHover = true): DepthPainter {
  ...
  function plan(w: number, h: number): Plan {
    ...
    if (!stack) {
      ...
      const k = Math.min((wlAnchor - 30) / maxAAll, (h - wlAnchor - 36) / maxBAll, kCap) * ICEBERG_SCALE * (isMobileCanvas ? 1.07 : 1.0)
...
export default function IcebergPackagesSection({ id }: { id?: string }) {
  ...
  if (isMobile) {
    ...
    const sharedFs = Math.min(...([0, 1, 2].map((j) => {
      ...
      const tO = Math.max(16, a.depth * 0.10)
      return Math.max(13, Math.min(16, (a.depth * 0.90 - tO) / TOTAL_EM[j]))
    })))
    const sharedTopOff = Math.min(...mobileImgs.anchors.map(a => Math.max(16, a.depth * 0.10)))
    return (
      <div
        id={id}
        style={{

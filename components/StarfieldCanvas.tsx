...
function randomStarXY(z: number): { x: number; y: number } {
  if ((window.innerWidth < 1024 ? z > 1400 : z > 2000) && Math.random() < (window.innerWidth < 1024 ? 0.80 : 0.60)) {
...
export default function StarfieldCanvas() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const mwAlpha = Math.min(1, Math.max(0, (window.innerWidth < 1024 ? (camZ - 1000) / 600 : (camZ - 1400) / 600))) * (window.innerWidth < 1024 ? 0.15 : 0.07)
      if (mwAlpha > 0.001) {
        ctx.save()
        ctx.translate(cx, cy)

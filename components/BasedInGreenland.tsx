...
export default function BasedInGreenland() {
  ...
  useEffect(() => {
    ...
    const startAnimation = async () => {
      ...
      const finalCY = isMobileNow ? H * 0.27 : H * 0.51
      const finalScale = isMobileNow ? Math.round(2500 * (window.innerWidth / 1728)) : Math.round(2050 * (window.innerWidth / 1728))
      ...
      const drawLoop = () => {
        ...
        if (p >= 0.86) {
          ...
          if (p < 0.22) {
            ...
            cy = lerp(isMobileNow ? 0.42 * H : H / 2, isMobileNow ? H * 0.27 : H * 0.51, q)
  ...
  return (
    <section
      ...
      id="based"
      data-snap="true"
      style={{ height: isMobile ? '650dvh' : '500vh' }}
    >
      <div
        style={{
          position: 'sticky',
